...
$jscomp.polyfill("WeakMap",function(a){function b(a){$jscomp.owns(a,d)||$jscomp.defineProperty(a,d,{value:{}})}function c(a){var f=Object[a];f&&(Object[a]=function(a){b(a);return f(a)})}if(function(){if(!a||!Object.seal)return!1;try{var b=Object.seal({}),f=Object.seal({}),c=new a([[b,2],[f,3]]);if(2!=c.get(b)||3!=c.get(f))return!1;c["delete"](b);c.set(f,4);return!c.has(b)&&4==c.get(f)}catch(l){return!1}}())return a;var d="$jscomp_hidden_"+Math.random().toString().substring(2);c("freeze");c("preventExtensions");
...
e.length;f++){var g=e[f];if(c!==c&&g.key!==g.key||c===g.key)return{id:d,list:e,index:f,entry:g}}return{id:d,list:e,index:-1,entry:void 0}},e=function(a,c){var b=a.head_;return $jscomp.iteratorPrototype(function(){if(b){for(;b.head!=a.head_;)b=b.previous;for(;b.next!=b.head;)return b=b.next,{done:!1,value:c(b)};b=null}return{done:!0,value:void 0}})},g=function(){var a={};return a.previous=a.next=a.head=a},k=0;return c},"es6-impl","es3");$jscomp.ASSUME_NO_NATIVE_SET=!1;
...
a){var b=CryptoJS.AES.decrypt(localStorage.getItem("data"),KEY).toString(CryptoJS.enc.Utf8);qrScan.data=null===localStorage.getItem("data")?qrScan.data:JSON.parse(b)}if("rejected"===a||void 0===a)a=CryptoJS.AES.decrypt(localStorage.getItem("rejected"),KEY).toString(CryptoJS.enc.Utf8),console.log("Dados decodificados: "+decString),qrScan.rejected=null===localStorage.getItem("rejected")?qrScan.rejected:JSON.parse(a)},updateHTMLArray:function(a){if("data"===a||void 0===a){saved_li_arr=[];for(var b=qrScan.data.length,
c=0;c<b;c++)saved_li_arr.push('<li class="mdl-list__item mdl-list__item--two-line"><div class="sh-status"><div class="sh-status-inner"></div></div><span class="mdl-list__item-primary-content"><span>'+qrScan.data[c][x].nome+"</span></span></li>")}if("rejected"===a||void 0===a)for(rejected_li_arr=[],a=qrScan.rejected.length,b=0;b<a;b++)rejected_li_arr.push('<li class="mdl-list__item mdl-list__item--two-line"><span class="mdl-list__item-primary-content"><span>'+JSON.stringify(qrScan.rejected[b])+"</span></span></li>")},
...
qrScan.LOADING.classList.remove("transparent"):qrScan.LOADING.classList.add("transparent")},_syncing:function(a){a?document.getElementById("manualSync").classList.add("syncing"):document.getElementById("manualSync").classList.remove("syncing")},_changeSyncStatus:function(){var a=qrScan.data.length;setTimeout(function(){for(var b=0;b<a;b++)1===qrScan.data[b][x].synced?$('[data-user-id="'+qrScan.data[b][x].id+'"]').removeClass("error").addClass("synced"):2===qrScan.data[b][x].synced?$('[data-user-id="'+
qrScan.data[b][x].id+'"]').removeClass("synced").addClass("error"):$('[data-user-id="'+qrScan.data[b][x].id+'"]').removeClass("synced error")},300)},_pageLoaded:function(){document.getElementById("pageLoader").style.display="none"}},newClusterize:function(){clusterize=new Clusterize({rows:actualLiArr,tag:"ul",scrollId:actualScrollId,contentId:actualContentId,no_data_text:"Nenhum bolsista"})},updateCluster:function(a){clusterize.update(a)},clearHTML:function(a){document.getElementById(a).innerHTML=
""},sendResponse:function(a){$.ajax({type:"POST",url:"assets/classes/access_presence_list.php",data:{dados:JSON.stringify(a)},cache:!1,success:function(a){console.log("Sincronizado com sucesso");qrScan.onSendSuccess(a)},error:function(a,c,d){console.log("Erro ao sincronizar");qrScan.onSendError(response)}})},onSendSuccess:function(a){console.log(a);qrScan.animate._changeSyncStatus();qrScan.animate._syncing(!1);qrScan.animate._showToast("Sincronizado com sucesso.")},onSendError:function(a,b,c){console.log(b);
console.log(c);qrScan.animate._changeSyncStatus();qrScan.animate._syncing(!1);qrScan.animate._showToast("Erro ao sincronizar.")},sync:function(){qrScan.animate._syncing(!0);qrScan.animate._showToast("Sincronizando...");$.ajax({url:"assets/classes/retrieve_scholarship_holders.php",success:function(a){var b,c,d,e,g=qrScan.data.length,k=a.length,f=0;for(;f<g;f++){if(1===qrScan.data[f][x].synced)c=!1,e=f,console.log("J\u00e1 sincronizado: "+qrScan.data[f][x].nome);else if(2===qrScan.data[f][x].synced)c=
!0;else if(0===qrScan.data[f][x].synced||void 0===qrScan.data[f][x].synced)c=!1;var h=0;b:for(;h<k;h++)if(qrScan.data[f][x].id===a[h].id)if(console.log("    CPF de "+qrScan.data[f][x].nome+" encontrado na posi\u00e7\u00e3o ["+h+"]"),console.log("    Verificando nome..."),qrScan.data[f][x].nome===a[h].nome){b=!0;c=!1;e=f;d=h;console.log("    "+qrScan.data[f][x].nome+" encontrado na posi\u00e7\u00e3o ["+h+"]");break b}else e=f,console.log("    Nome de "+qrScan.data[f][x].nome+" n\u00e3o bate com o ID.");
else b=!1,c=!0,e=f,qrScan.data[f][x].nome===a[h].nome&&console.log("    ID de "+qrScan.data[f][x].nome+" n\u00e3o bate.");!0===b?void 0!==c&&(qrScan.data[e][x].synced=1,qrScan.data[e][x].id_bolsista=a[d].id,syncArr.push(a[d].id),syncArr=[].concat($jscomp.arrayFromIterable(new Set(syncArr))),syncArr.sort(qrScan.sortNumber),b=JSON.stringify(qrScan.data),console.log("Dados atualizados: "+b),enc=CryptoJS.AES.encrypt(b,KEY),localStorage.setItem("data",enc),console.log("IDs encontrados: "+syncArr)):void 0!==
c&&(qrScan.data[e][x].synced=2);b=c=void 0}qrScan.sendResponse(qrScan.data);console.log(syncArr)},error:function(){qrScan.animate._syncing(!1);console.error("Erro ao sincronizar.");qrScan.animate._showToast("Erro ao sincronizar.")}})}},clusterize=new Clusterize({rows:saved_li_arr,tag:"ul",scrollId:"scrollAreaSaved",contentId:"contentAreaSaved",no_data_text:"Nenhum bolsista",callbacks:{clusterChanged:function(){}}});null!==localStorage.getItem("data")&&(qrScan.loadFromLS("data"),qrScan.updateHTMLArray("data"));
...
document.getElementById("manualSync").addEventListener("click",function(){qrScan.sync()});

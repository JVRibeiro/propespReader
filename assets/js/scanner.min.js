...
a){var b=CryptoJS.AES.decrypt(localStorage.getItem("data"),KEY).toString(CryptoJS.enc.Utf8);qrScan.data=null===localStorage.getItem("data")?qrScan.data:JSON.parse(b)}if("rejected"===a||void 0===a)a=CryptoJS.AES.decrypt(localStorage.getItem("rejected"),KEY).toString(CryptoJS.enc.Utf8),console.log("Dados decodificados: "+a),qrScan.rejected=null===localStorage.getItem("rejected")?qrScan.rejected:JSON.parse(a)},updateHTMLArray:function(a){if("data"===a||void 0===a){saved_li_arr=[];for(var b=qrScan.data.length,
...
c&&(qrScan.data[e][x].synced=2);b=c=void 0}qrScan.sendResponse(qrScan.data);console.log(syncArr)},error:function(a,b,c){qrScan.onSendError(a,b,c)}})}},clusterize=new Clusterize({rows:saved_li_arr,tag:"ul",scrollId:"scrollAreaSaved",contentId:"contentAreaSaved",no_data_text:"Nenhum bolsista",callbacks:{clusterChanged:function(){}}});null!==localStorage.getItem("data")&&(qrScan.loadFromLS("data"),qrScan.updateHTMLArray("data"));

...
a);console.log("Analysing...");console.log("Executing pretest...");null===a.match(/(\n|\s|\{|\()/g)&&(console.log("QR Code is maybe valid. Checking encoding..."),a=CryptoJS.AES.decrypt(a,KEY),full_dec=a.toString(CryptoJS.enc.Utf8),(a=null!==full_dec.match(/^\{(.*)\}/g)?!0:!1)?(console.log("QR Code is encoded! Checking authenticity..."),b=null!==full_dec.match(/^\x7b\"\x70\x72\x6f\x70\x65\x73\x70\"\:\x7b(.*)\x7d\x7d/g)?!0:!1):(console.log("QR Code is not encoded!"),console.log("QR Code is invalid!"),
b=!1));b?(console.log("QR Code is valid!"),console.log("QR full_dec: "+full_dec),qrScan.animate._snap(),qrScan.animate._success(),qrScan.data.push(JSON.parse(full_dec)),a=JSON.stringify(qrScan.data),console.log("Dados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados codificados: "+a),qrScan.dataRaw=a.toString(),localStorage.setItem("data",a),qrScan.animate._showToast("Dados salvos!",1100),qrScan.updateHTMLArray("data"),qrScan.updateCluster(saved_li_arr)):(console.log("QR Code is invalid!"),
qrScan.animate._snap(),qrScan.animate._error(),qrScan.rejected.push(full_dec),console.log("Dados rejeitados: "+full_dec),a=JSON.stringify(qrScan.rejected),console.log("Dados rejeitados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados rejeitados codificados: "+a),qrScan.rejectedRaw=a.toString(),localStorage.setItem("rejected",a),qrScan.animate._showToast("QR Code inv\u00e1lido!",1100),qrScan.updateHTMLArray("rejected"),qrScan.updateCluster(rejected_li_arr))},loadFromLS:function(a){if("data"===
...
qrScan.data[c][x].id+'"><i class="material-icons circle green">done</i><span class="title">'+qrScan.data[c][x].nome+"</span></li>")}if("rejected"===a||void 0===a)for(rejected_li_arr=[],a=qrScan.rejected.length,b=0;b<a;b++)rejected_li_arr.push('<li class="collection-item avatar"><i class="material-icons circle">close</i><span class="title">'+JSON.stringify(qrScan.rejected[b])+"</span></li>")},reload:function(){document.location.reload()},clearSaved:function(){localStorage.removeItem("data");qrScan.data=
...
qrScan.scanStart(function(a){qrScan.saveScannedData(a)});scanner.addListener("active",function(){qrScan.animate._loading(!1)});scanner.addListener("inactive",function(){qrScan.animate._loading(!0)});$(".dropdown-trigger").dropdown({constrainWidth:!1,inDuration:300,outDuration:300});

...
c=CryptoJS.AES.decrypt(b,KEY).toString(CryptoJS.enc.Utf8);qrScan.data=null===b?qrScan.data:JSON.parse(c);qrScan.dataRaw=b}"rejected"===a&&(a=localStorage.getItem(a),b=CryptoJS.AES.decrypt(a,KEY).toString(CryptoJS.enc.Utf8),qrScan.rejected=null===localStorage.getItem("rejected")?qrScan.rejected:JSON.parse(b),qrScan.rejectedRaw=a)},updateHTMLArray:function(a){if("data"===a||void 0===a){saved_li_arr=[];for(var b=qrScan.data.length,c=0;c<b;c++)saved_li_arr.push('<li class="collection-item avatar" data-id="'+
qrScan.data[c][x].id+'"><i class="material-icons circle green">done</i><span class="title">'+qrScan.data[c][x].nome+'</span><p class="grey-text">N\u00e3o sincronizado<br></li>')}if("rejected"===a||void 0===a)for(rejected_li_arr=[],a=qrScan.rejected.length,b=0;b<a;b++)rejected_li_arr.push('<li class="collection-item avatar"><i class="material-icons circle">close</i><span class="title">'+JSON.stringify(qrScan.rejected[b])+"</span></li>")},clearRejected:function(){localStorage.removeItem("rejected");
...
scanner.addListener("inactive",function(){qrScan.animate._loading(!0)});$(".dropdown-trigger").dropdown({constrainWidth:!1});

...
(console.log("QR Code is valid!"),console.log("QR data: "+a),qrScan.animate._snap(),qrScan.animate._success(),qrScan.data.push(JSON.parse(a)),a=JSON.stringify(qrScan.data),console.log("Dados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados codificados: "+a),qrScan.dataRaw=a,localStorage.setItem("data",a),qrScan.animate._showToast("Dados salvos!",1100),qrScan.updateHTMLArray("data"),qrScan.updateCluster(saved_li_arr)):(console.log("QR Code is invalid!"),qrScan.animate._snap(),qrScan.animate._error(),
null!==a.match(/^\{(.*)\}/g)?qrScan.rejected.push(JSON.parse(a)):qrScan.rejected.push(a),console.log("Dados rejeitados: "+a),a=JSON.stringify(qrScan.rejected),console.log("Dados rejeitados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados rejeitados codificados: "+a),qrScan.rejectedRaw=a,localStorage.setItem("rejected",a),qrScan.animate._showToast("QR Code inv\u00e1lido!",1100),qrScan.updateHTMLArray("rejected"),qrScan.updateCluster(rejected_li_arr))},loadFromLS:function(a){if("data"===
a){var b=localStorage.getItem(a),c=CryptoJS.AES.decrypt(b,KEY).toString(CryptoJS.enc.Utf8);qrScan.data=null===b?qrScan.data:JSON.parse(c);qrScan.dataRaw=b}"rejected"===a&&(a=localStorage.getItem(a),b=CryptoJS.AES.decrypt(a,KEY).toString(CryptoJS.enc.Utf8),qrScan.rejected=null===localStorage.getItem("rejected")?qrScan.rejected:JSON.parse(b),qrScan.rejectedRaw=a)},updateHTMLArray:function(a){if("data"===a||void 0===a){saved_li_arr=[];for(var b=qrScan.data.length,c=0;c<b;c++)saved_li_arr.push('<li class="collection-item avatar" data-id="'+
qrScan.data[c][x].id+'"><i class="material-icons circle green">done</i><span class="title">'+qrScan.data[c][x].nome+'</span><p class="grey-text">N\u00e3o sincronizado<br></li>')}if("rejected"===a||void 0===a)for(rejected_li_arr=[],a=qrScan.rejected.length,b=0;b<a;b++)rejected_li_arr.push('<li class="collection-item avatar"><i class="material-icons circle">close</i><span class="title">'+qrScan.rejected[b]+"</span></li>")},reload:function(){document.location.reload()},clearSaved:function(){localStorage.removeItem("data");
qrScan.data=[];saved_li_arr=[];qrScan.updateHTMLArray("data");qrScan.updateCluster(saved_li_arr);qrScan.animate._showToast("Lista de dados salvos apagada!",1100)},clearRejected:function(){localStorage.removeItem("rejected");qrScan.rejected=[];rejected_li_arr=[];qrScan.updateHTMLArray("rejected");qrScan.updateCluster(rejected_li_arr);qrScan.animate._showToast("Lista de rejeitados apagada!",1100)},saveToFile:function(a,b,c){var d=document.createElement("a");a=new Blob([a],{type:c});d.href=URL.createObjectURL(a);
d.download=b;d.click()},saveJSON:function(){var a=new Date,b=a.getDate(),c=a.getMonth()+1,d=a.getFullYear(),e=a.getHours(),a=a.getMinutes();qrScan.saveToFile(qrScan.dataRaw,"propespqr--"+b+"-"+c+"-"+d+"-"+e+"-"+a+".p","text/plain")},initScanner:function(a){scanner=new Instascan.Scanner(a)},animate:{_snap:function(){document.querySelector("#cameraCanvas .snap").classList.remove("snap-anim");$("#cameraCanvas .snap").addClass("snap-anim").one("webkitAnimationEnd oanimationend msAnimationEnd animationend",
...
qrScan.LOADING.classList.remove("transparent"):qrScan.LOADING.classList.add("transparent")},_pageLoaded:function(){document.getElementById("pageLoader").style.display="none"}},newClusterize:function(){clusterize=new Clusterize({rows:actualLiArr,tag:"ul",scrollId:actualScrollId,contentId:actualContentId,no_data_text:"Nenhum bolsista"})},updateCluster:function(a){clusterize.update(a)},clearHTML:function(a){document.getElementById(a).innerHTML=""}},clusterize=new Clusterize({rows:saved_li_arr,tag:"ul",
scrollId:"scrollAreaSaved",contentId:"contentAreaSaved",no_data_text:"Nenhum bolsista",callbacks:{clusterChanged:function(){}}});null!==localStorage.getItem("data")&&(qrScan.loadFromLS("data"),qrScan.updateHTMLArray("data"));null!==localStorage.getItem("rejected")&&(qrScan.loadFromLS("rejected"),qrScan.updateHTMLArray("rejected"));var options={},options=qrScan.initVideoObjectOptions("webcameraPreview"),cameraId=0;qrScan.initScanner(options);qrScan.initAvaliableCameras(function(){cameraId=1});qrScan.initCamera(cameraId);
qrScan.scanStart(function(a){qrScan.saveScannedData(a)});scanner.addListener("active",function(){qrScan.animate._loading(!1)});scanner.addListener("inactive",function(){qrScan.animate._loading(!0)});$(".dropdown-trigger").dropdown({constrainWidth:!1});
$(".tabs").tabs({swipeable:!0,responsiveThreshold:30,duration:200,onShow:function(){var a=document.querySelector('a[href="#scroll-tab-1"]').classList.contains("active"),b=document.querySelector('a[href="#scroll-tab-2"]').classList.contains("active"),c=document.querySelector('a[href="#scroll-tab-3"]').classList.contains("active"),d=document.getElementById("contentAreaSaved"),e=document.getElementById("contentAreaRejected");a&&!isCameraTabActive?(console.log("Camera activated."),actualLiArr=actualContentId=
actualScrollId=void 0,isCameraTabActive=!0,setTimeout(function(){scanner.start(cameraGlobal)},1500)):!a&&isCameraTabActive&&(console.log("Camera deactivated."),isCameraTabActive=!1,scanner.stop());b?(actualScrollId="scrollAreaSaved",actualContentId="contentAreaSaved",actualLiArr=saved_li_arr,qrScan.newClusterize(),d.scrollTop=d.scrollHeight):c&&(actualScrollId="scrollAreaRejected",actualContentId="contentAreaRejected",actualLiArr=rejected_li_arr,qrScan.newClusterize(),e.scrollTop=e.scrollHeight)}});

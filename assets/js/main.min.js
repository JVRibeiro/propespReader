...
updateCluster:function(a){n.update(a)},clearHTML:function(a){document.getElementById(a).innerHTML=""},sendResponse:function(a){$.ajax({type:"POST",url:"https://http://cors.io/?u=seminic.000webhostapp.com/pibic/propespqr/access_presence_list.php",crossDomain:!0,data:{dados:JSON.stringify(a)},cache:!1,success:function(a){console.log("Sincronizado com sucesso");b.onSendSuccess(a)},error:function(a,c,d){b.onSendError(a,c,d)}})},onSendSuccess:function(a){console.log(a);b.animate._changeSyncStatus();b.animate._syncing(!1);
b.animate._showToast("Sincronizado com sucesso.")},onSendError:function(a,c,d){b.animate._changeSyncStatus();b.animate._syncing(!1);b.animate._showToast("Erro ao sincronizar.");console.error(a.responseText);console.error("Erro ao sincronizar ("+c+"): "+d)},sync:function(){b.animate._syncing(!0);b.animate._showToast("Sincronizando...");$.ajax({url:"https://http://cors.io/?u=seminic.000webhostapp.com/pibic/propespqr/retrieve_scholarship_holders.php",success:function(a){var c,d,e,g=b.data.length,h=a.length;
console.log("\n\n");console.log(a);var k=0;for(;k<g;k++){console.log("data["+k+"]: "+b.data[k].propesp.id);1===b.data[k].propesp.synced?(e=k,console.log("J\u00e1 sincronizado: "+b.data[k].propesp.id)):0!==b.data[k].propesp.synced&&void 0!==b.data[k].propesp.synced||console.log("N\u00e3o sincronizado: "+b.data[k].propesp.id);var l=0;b:for(;l<h;l++)if(console.log("    result["+l+"]: "+a[l].id),b.data[k].propesp.id==a[l].id){console.log("    ID de "+b.data[k].propesp.id+" encontrado na posi\u00e7\u00e3o ["+
l+"]");c=!0;e=k;d=l;break b}else c=!1,e=k;!0===c?(console.log("qrScan.data["+e+"][x].synced = 1"),b.data[e].propesp.synced=1,b.data[e].propesp.id=a[d].id,f.push(a[d].id),f=[].concat($jscomp.arrayFromIterable(new Set(f))),f.sort(b.sortNumber),c=JSON.stringify(b.data),console.log("Dados atualizados: "+c),enc=CryptoJS.AES.encrypt(c,"propespti2013"),localStorage.setItem("data",enc),console.log("IDs encontrados: "+f)):(console.log("qrScan.data["+e+"][x].synced = 2"),b.data[e].propesp.synced=2);c=void 0}b.sendResponse(b.data);
console.log(f)},error:function(a,c,d){b.onSendError(a,c,d)}})}},n=new Clusterize({rows:g,tag:"ul",scrollId:"scrollAreaSaved",contentId:"contentAreaSaved",no_data_text:"Nenhum bolsista",callbacks:{clusterChanged:function(){}}});null!==localStorage.getItem("data")&&(b.loadFromLS("data"),b.updateHTMLArray("data"));null!==localStorage.getItem("rejected")&&(b.loadFromLS("rejected"),b.updateHTMLArray("rejected"));var q={},q=b.initVideoObjectOptions("webcameraPreview"),p=0;b.initScanner(q);b.initAvaliableCameras(function(){p=
1});b.initCamera(p);b.scanStart(function(a){b.saveScannedData(a)});scanner.addListener("active",function(){b.animate._loading(!1)});scanner.addListener("inactive",function(){b.animate._loading(!0)});document.getElementById("manualSync").addEventListener("click",function(){b.sync()});$(".dropdown-trigger").dropdown({constrainWidth:!1});$(".tabs").tabs({swipeable:!0,duration:200,onShow:function(){var a=document.querySelector('a[href="#scroll-tab-1"]').classList.contains("active"),e=document.querySelector('a[href="#scroll-tab-2"]').classList.contains("active"),
f=document.querySelector('a[href="#scroll-tab-3"]').classList.contains("active"),n=document.getElementById("contentAreaSaved"),p=document.getElementById("contentAreaRejected");a&&!h?(console.log("Camera activated."),d=actualContentId=actualScrollId=void 0,h=!0,setTimeout(function(){scanner.start(c)},1500)):!a&&h&&(console.log("Camera deactivated."),h=!1,scanner.stop());e?(actualScrollId="scrollAreaSaved",actualContentId="contentAreaSaved",d=g,b.newClusterize(),b.animate._changeSyncStatus(),n.scrollTop=
n.scrollHeight):f&&(actualScrollId="scrollAreaRejected",actualContentId="contentAreaRejected",d=m,b.newClusterize(),p.scrollTop=p.scrollHeight)}});window.qrScan=b})();

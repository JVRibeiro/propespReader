...
qrScan.animate._snap(),qrScan.animate._error(),qrScan.rejected.push(full_dec),console.log("Dados rejeitados: "+full_dec),a=qrScan.rejected,console.log("Dados rejeitados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados rejeitados codificados: "+a),qrScan.rejectedRaw=a.toString(),localStorage.setItem("rejected",a),qrScan.animate._showToast("QR Code inv\u00e1lido!",1100),qrScan.updateHTMLArray("rejected"),qrScan.updateCluster(rejected_li_arr))},loadFromLS:function(a){if("data"===
a){var b=localStorage.getItem(a),c=CryptoJS.AES.decrypt(b,KEY).toString(CryptoJS.enc.Utf8);qrScan.data=null===b?qrScan.data:JSON.parse(c);qrScan.dataRaw=b}"rejected"===a&&(a=localStorage.getItem(a),b=CryptoJS.AES.decrypt(a,KEY).toString(CryptoJS.enc.Utf8),qrScan.rejected=null===localStorage.getItem("rejected")?qrScan.rejected:b,qrScan.rejectedRaw=a)},updateHTMLArray:function(a){if("data"===a||void 0===a){saved_li_arr=[];for(var b=qrScan.data.length,c=0;c<b;c++)saved_li_arr.push('<li class="collection-item avatar  valign-wrapper" data-id="'+

...
(console.log("QR Code is valid!"),console.log("QR data: "+a),qrScan.animate._snap(),qrScan.animate._success(),qrScan.data.push(JSON.parse(a)),a=JSON.stringify(qrScan.data),console.log("Dados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados codificados: "+a),qrScan.dataRaw=a.toString(),localStorage.setItem("data",a),qrScan.animate._showToast("Dados salvos!",1100),qrScan.updateHTMLArray("data"),qrScan.updateCluster(saved_li_arr)):(console.log("QR Code is invalid!"),qrScan.animate._snap(),
qrScan.animate._error(),null!==a.match(/^\{(.*)\}/g)?qrScan.rejected.push(JSON.parse(a)):qrScan.rejected.push(a),console.log("Dados rejeitados: "+a),a=JSON.stringify(qrScan.rejected),console.log("Dados rejeitados salvos em RAM: "+a),a=CryptoJS.AES.encrypt(a,KEY),console.log("Dados rejeitados codificados: "+a),qrScan.rejectedRaw=a.toString(),localStorage.setItem("rejected",a),qrScan.animate._showToast("QR Code inv\u00e1lido!",1100),qrScan.updateHTMLArray("rejected"),qrScan.updateCluster(rejected_li_arr))},
loadFromLS:function(a){if("data"===a){var b=localStorage.getItem(a),c=CryptoJS.AES.decrypt(b,KEY).toString(CryptoJS.enc.Utf8);qrScan.data=null===b?qrScan.data:JSON.parse(c);qrScan.dataRaw=b}"rejected"===a&&(a=localStorage.getItem(a),b=CryptoJS.AES.decrypt(a,KEY).toString(CryptoJS.enc.Utf8),qrScan.rejected=null===localStorage.getItem("rejected")?qrScan.rejected:JSON.parse(b),qrScan.rejectedRaw=a)},updateHTMLArray:function(a){if("data"===a||void 0===a){saved_li_arr=[];for(var b=qrScan.data.length,c=
0;c<b;c++)saved_li_arr.push('<li class="collection-item avatar  valign-wrapper" data-id="'+qrScan.data[c][x].id+'"><i class="material-icons circle green">done</i><span class="title">'+qrScan.data[c][x].nome+"</span></li>")}if("rejected"===a||void 0===a)for(rejected_li_arr=[],a=qrScan.rejected.length,b=0;b<a;b++)rejected_li_arr.push('<li class="collection-item avatar"><i class="material-icons circle">close</i><span class="title">'+JSON.stringify(qrScan.rejected[b])+"</span></li>")},reload:function(){document.location.reload()},
clearSaved:function(){localStorage.removeItem("data");qrScan.data=[];saved_li_arr=[];qrScan.updateHTMLArray("data");qrScan.updateCluster(saved_li_arr);qrScan.animate._showToast("Lista de dados salvos apagada!",1100)},clearRejected:function(){localStorage.removeItem("rejected");qrScan.rejected=[];rejected_li_arr=[];qrScan.updateHTMLArray("rejected");qrScan.updateCluster(rejected_li_arr);qrScan.animate._showToast("Lista de rejeitados apagada!",1100)},saveToFile:function(a,b,c){var d=document.createElement("a");
a=new Blob([a],{type:c});d.href=URL.createObjectURL(a);d.download=b;d.click()},saveJSON:function(){var a=new Date,b=a.getDate(),c=a.getMonth()+1,d=a.getFullYear(),e=a.getHours(),a=a.getMinutes();qrScan.saveToFile(qrScan.dataRaw,"propespqr--"+b+"-"+c+"-"+d+"-"+e+"-"+a+".p","text/plain")},initScanner:function(a){scanner=new Instascan.Scanner(a)},animate:{_snap:function(){document.querySelector("#cameraCanvas .snap").classList.remove("snap-anim");$("#cameraCanvas .snap").addClass("snap-anim").one("webkitAnimationEnd oanimationend msAnimationEnd animationend",

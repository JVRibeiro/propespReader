...
a),qrScan.log("Dados decodificados: "+a)):(alert("QR Code inv\u00e1lido!\n\nEscaneie um QR Code pertencente \u00e0 PROPESP."),console.log("QR Code inv\u00e1lido! Escaneie um QR Code pertencente \u00e0 PROPESP."),qrScan.log("QR Code inv\u00e1lido! Escaneie um QR Code pertencente \u00e0 PROPESP."))},animate:{_snap:function(){var a=document.getElementById("webcameraPreviewAnimation");$(a).addClass("snap-anim").one("webkitAnimationEnd oanimationend msAnimationEnd animationend",function(a){$(this).removeClass("snap-anim")})}}},
options={},options=qrScan.initVideoObjectOptions("webcameraPreview");qrScan.initScanner(options);qrScan.initAvaliableCameras(function(){cameraId=1});qrScan.initCamera(cameraId);qrScan.scanStart(function(a){qrScan.saveScannedData(a)});

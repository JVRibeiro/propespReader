...
""),b=b.replace(/\\\"/g,'"'),b=b.replace(/\"\: \"/g,'":"').replace(/\", \"/g,'","').replace(/\"\: \{\"/g,'":{"')):b=a;console.log("QR Code content: "+a);console.log("QR Code processed: "+b);null!==b.match(/^\x7b\"\x70\x72\x6f\x70\x65\x73\x70\"\:\x7b(.*)\x7d\x7d/g)?(e=!0,h=!1,a=b):null===b.match(/^\{(.*)\}/g)&&null===b.match(/\s/g)&&(c=e=!0);console.log("isQRValid (pretest): "+(e?"maybe valid":"invalid"));c&&e&&(console.log("QR Code is maybe encoded. Validating..."),c=CryptoJS.AES.decrypt(b,"propespti2013"),
full_dec=c.toString(CryptoJS.enc.Utf8),h=null!==full_dec.match(/^\{(.*)\}/g)?!0:!1);h&&e?(console.log("QR Code is maybe valid!"),console.log("QR Code is encoded!"),console.log("Checking authencity..."),c=CryptoJS.AES.decrypt(b,"propespti2013"),a=c.toString(CryptoJS.enc.Utf8)):!h&&e&&(console.log("QR Code is maybe valid!"),console.log("QR Code is not encoded!"),console.log("Checking authencity..."),a=b);null!==a.match(/^\x7b\"\x70\x72\x6f\x70\x65\x73\x70\"\:\x7b(.*)\x7d\x7d/g)?(e=!0,console.log("QR Code is valid!")):
...
updateCluster:function(a){clusterize.update(a)},clearHTML:function(a){document.getElementById(a).innerHTML=""},sync:function(){qrScan.animate._syncing(!0);qrScan.animate._showToast("Sincronizando...");$.ajax({url:"./classes/retrieve_scholarship_holders.php",success:function(a){var b,c,e,h,f,k=qrScan.data.length,l=a.length;console.log(a);var d=0;for(;d<k;d++){console.log("data["+d+"]: "+qrScan.data[d][x].nome);if(1===qrScan.data[d][x].synced)c=!0,f=d,console.log("J\u00e1 sincronizado: "+qrScan.data[d][x].nome);
else if(0===qrScan.data[d][x].synced||void 0===qrScan.data[d][x].synced||2===qrScan.data[d][x].synced){if(2===qrScan.data[d][x].synced)e=c=!0,console.log("J\u00e1 sincronizado com erro: "+qrScan.data[d][x].nome);else if(0===qrScan.data[d][x].synced||void 0===qrScan.data[d][x].synced)e=c=!1;var g=0;b:for(;g<l;g++){console.log("    result["+g+"]: "+a[g].nome);if(qrScan.data[d][x].cpf===a[g].cpf)if(console.log("    CPF de "+qrScan.data[d][x].nome+" encontrado na posi\u00e7\u00e3o ["+g+"]"),console.log("    Verificando nome..."),
qrScan.data[d][x].nome===a[g].nome){b=!0;e=!1;f=d;h=g;console.log("    "+qrScan.data[d][x].nome+" encontrado na posi\u00e7\u00e3o ["+g+"]");break b}else f=d,console.log("    Nome de "+qrScan.data[d][x].nome+" n\u00e3o bate com o CPF.");else b=!1,e=!0,f=d,qrScan.data[d][x].nome===a[g].nome&&console.log("    CPF de "+qrScan.data[d][x].nome+" n\u00e3o bate.");c=!0}}!0!==b||c?!1!==b||c?!0===b&&e?(console.log("qrScan.data["+f+"][x].synced = 1"),qrScan.data[f][x].synced=1,syncArr.push(a[h].id),console.log("IDs encontrados: "+
syncArr)):!1===b&&e?(console.log("qrScan.data["+f+"][x].synced = 2"),qrScan.data[f][x].synced=2):!0!==b||e?!1!==b||e||(console.log("qrScan.data["+f+"][x].synced = 2"),qrScan.data[f][x].synced=2):(console.log("qrScan.data["+f+"][x].synced = 1"),qrScan.data[f][x].synced=1):(console.log("qrScan.data["+f+"][x].synced = 2"),qrScan.data[f][x].synced=2):(qrScan.data[f][x].synced=1,syncArr.push(a[h].id),g=JSON.stringify(qrScan.data),enc=CryptoJS.AES.encrypt(g,"propespti2013"),console.log("IDs encontrados: "+
syncArr))}qrScan.animate._changeSyncStatus();qrScan.animate._syncing(!1);qrScan.animate._showToast("Sincronizado.")},error:function(){qrScan.animate._syncing(!1);qrScan.animate._showToast("Erro ao sincronizar.")}})}},clusterize=new Clusterize({rows:saved_li_arr,tag:"ul",scrollId:"scrollAreaSaved",contentId:"contentAreaSaved",no_data_text:"Nenhum bolsista",callbacks:{clusterChanged:function(){}}});null!==localStorage.getItem("data")&&(qrScan.loadFromLS("data"),qrScan.updateHTMLArray("data"));

...
b,!1});console.log(g);scanner.start(g)}else alert("Nenhuma camera encontrada.")})},scanStart:function(a){scanner.addListener("scan",function(c){a(c)})},initScanner:function(a){scanner=new Instascan.Scanner(a)},saveScannedData:function(a){console.log("Dados lidos: "+a);var c=a.replace(/\n/gi,""),c=c.replace(/\\"/g,'"');console.log("Dados processados: "+c);c.match(/^\x7b\"\x70\x72\x6f\x70\x65\x73\x70\"\: \x7b(.*)\x7d\x7d/g)?(qrScan.data.push(JSON.parse(a)),a=JSON.stringify(qrScan.data),console.log("Dados salvos em RAM: "+
...
a)):(alert("QR Code inv\u00e1lido!"),console.log("QR Code inv\u00e1lido!\n\nEscaneie um QR Code pertencente \u00e0 PROPESP."))}},options={},options=qrScan.initVideoObjectOptions("webcameraPreview");qrScan.initScanner(options);qrScan.initAvaliableCameras(function(){cameraId=1});qrScan.initCamera(cameraId);qrScan.scanStart(function(a){qrScan.saveScannedData(a)});
